            ...
            for name, d in self.callbacks.items():
                ...
                for type_ in d.values():
                    if type_ == "int":
                        ...
                        args_list.append("constants dict &error")
                    else:
                        args_list.append("constants dict &data")
    ...
    def generate_header_function(self) -> None:
        ...
        with open(filename, "w") as f:
            for name in self.functions.keys():
                ...
                line = f"int {name}(constants dict &req, int reqid);\n\n"
                f.write(line)
    ...
    def generate_source_function(self) -> None:
        ...
        with open(filename, "w") as f:
            for name, d in self.functions.items():
                ...
                f.write(
                    f"int {self.class_name}::{req_name}(constants dict &req, int reqid)\n")
                f.write("{\n")
            ...
            for name, d in self.callbacks.items():
                ...
                for _, type_ in d.items():
                    if type_ == "int":
                        ...
                        args.append("constants dict &error")
                        bind_args.append("error")
                    else:
                        args.append("constants dict &data")
                ...
                f.write("\t}\n")
                f.write("\tcatch (constants error_already_set &e)\n")
                f.write("\t{\n")
