...
class MovingAverageStrategy(BaseStrategy):
    ...
    def __init__(self, strategy_id: str, event_bus, external_params: Dict[str, Any] = None):
    ...
    async def on_init(self):
        ...
        self.write_log(f"策略参数: {self.params.symbol}.{self.params.exchange.value}, "
                      f"短期={self.params.short_window}, "
                      f"长期={self.params.long_window}, "
                      f"手数={self.params.volume}")
    ...
    async def on_start(self):
        ...
        # 订阅行情数据
        await self.subscribe_symbols([self.params.symbol])
    ...
    async def on_tick(self, tick: TickData):
        ...
        # 保持价格历史在合理长度
        if len(self.prices) > self.params.long_window * 2:
            self.prices = self.prices[-self.params.long_window * 2:]
        
        # 计算移动平均线
        if len(self.prices) >= self.params.long_window:
            self.short_ma = sum(self.prices[-self.params.short_window:]) / self.params.short_window
            self.long_ma = sum(self.prices[-self.params.long_window:]) / self.params.long_window
    ...
    async def on_buy_signal(self, current_price: float):
        """处理买入信号"""
        # 检查是否已有持仓
        if self.position >= self.params.max_positions:
    ...
    async def open_long_position(self, price: float):
        ...
        order_request = OrderRequest(
            ...
            exchange=self.params.exchange.value,
        ...
        if order_id:
            ...
            self.write_log(f"发送开多订单: {self.params.volume}@{price}")
    ...
    async def open_short_position(self, price: float):
        ...
        if order_id:
            self.active_orders[order_id] = order_request
            self.write_log(f"发送开空订单: {self.params.volume}@{price}")
    ...
    async def close_all_positions(self):
        ...
        if self.position > 0:
            # 获取最新价格
            latest_tick = self.get_latest_tick(self.params.symbol)
    ...
    async def check_stop_loss_take_profit(self, current_price: float):
        ...
        # 止盈
        elif pnl_ratio >= self.params.take_profit:
