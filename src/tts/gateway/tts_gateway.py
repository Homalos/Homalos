...
from .tts_gateway_helper import tts_build_contract
...
from ...core.event import TickEvent, TradeUpdateEvent, OrderUpdateEvent, PositionUpdateEvent, AccountUpdateEvent, \
    ContractInfoEvent
from ...core.event_engine import EventEngine
from src.config.constants import Direction, Exchange, Offset, OptionType, OrderType, Product, Status
...
from ...core.object import (
    ...
    SubscribeRequest,
    TickData,
    TradeData, GatewayConnectionStatus,
)
...
from ...ctp.gateway.ctp_mapping import EXCHANGE_CTP2VT
...
class TtsGateway(BaseGateway):
    ...
    def __init__(self, gateway_name: str, event_engine: EventEngine) -> None:
        ...
        try:
            ...
            map_file_path = os.path.join(current_dir, "..", "..", "config", "project_files",
                                         "instrument_exchange_id.json")
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        if req.type == OrderType.RFQ:
            ho_orderid: str = self.td_api.send_rfq(req)
        # 其他委托
        else:
            ho_orderid: str = self.td_api.send_order(req)
        return ho_orderid
    ...
    def on_position(self, position: PositionData) -> None:
        """持仓推送"""
        if self.event_engine:
            asyncio.run_coroutine_threadsafe(self.event_engine.put(PositionUpdateEvent(data=position)),
                                             self.main_loop)
        else:
    ...
    def on_account(self, account: AccountData) -> None:
        """资金推送"""
        if self.event_engine:
            asyncio.run_coroutine_threadsafe(self.event_engine.put(AccountUpdateEvent(data=account)),
                                             self.main_loop)
        else:
    ...
    def on_contract(self, contract: ContractData) -> None:
        """合约推送"""
        if self.event_engine:
            asyncio.run_coroutine_threadsafe(self.event_engine.put(ContractInfoEvent(data=contract)),
                                             self.main_loop)
        else:
...
class TtsMdApi(MdApi):
    ...
    def onRspUserLogin(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        if not error["ErrorID"]:
            ...
            self.gateway.on_gateway_status(GatewayConnectionStatus.ERROR,
                                           f"行情服务器登录失败: {error.get('ErrorMsg', '未知错误')}")
    ...
    def onRtnDepthMarketData(self, data: dict) -> None:
        ...
        if json_exchange_str:
            try:
                ...
                self.gateway.write_log(
                    f"CtpMdApi: Determined exchange for {instrument_id} as {exchange_enum} from JSON map ('{json_exchange_str}').",
                    "DEBUG")
            except KeyError:
                self.gateway.write_log(
                    f"CtpMdApi: Invalid exchange string '{json_exchange_str}' in JSON map for {instrument_id}. Trying CTP ExchangeID.",
                    "WARNING")
        else:
            self.gateway.write_log(
                f"CtpMdApi: InstrumentID {instrument_id} not found in JSON map. Trying CTP ExchangeID.", "DEBUG")
        ...
        if not exchange_enum:
            ...
            if ctp_exchange_id_value and ctp_exchange_id_value != "":
                ...
                if exchange_enum:
                    self.gateway.write_log(
                        f"CtpMdApi: Determined exchange for {instrument_id} as {exchange_enum} from CTP ExchangeID ('{exchange_str_from_ctp}').",
                        "DEBUG")
                else:
                    self.gateway.write_log(
                        f"CtpMdApi: Unknown CTP ExchangeID '{exchange_str_from_ctp}' for {instrument_id}. Available CTP_IDs: {list(EXCHANGE_CTP2VT.keys())}",
                        "WARNING")
            else:
                self.gateway.write_log(f"CtpMdApi: CTP ExchangeID is empty or not provided for {instrument_id}.",
                                       "DEBUG")

        # 最后检查：如果在所有回退之后仍无法确定交换，则删除勾选。
        if not exchange_enum:
            self.gateway.write_log(
                f"CtpMdApi: CRITICAL: Unable to determine exchange for {instrument_id} after all fallbacks. Dropping tick. JSON_Map_Exchange: '{json_exchange_str}', CTP_ID_was: '{data.get('ExchangeID')}'")
            return

        # 对大商所的交易日字段取本地日期
        if not data["ActionDay"] or (
        exchange_enum == Exchange.DCE if exchange_enum else False):  # Check resolved exchange
...
class TtsTdApi(TdApi):
    ...
    def onRspAuthenticate(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        if not error['ErrorID']:
            ...
            self.gateway.on_gateway_status(GatewayConnectionStatus.ERROR,
                                           f"交易服务器授权失败: {error.get('ErrorMsg', '未知错误')}")
    ...
    def onRspUserLogin(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        if not error["ErrorID"]:
            ...
            self.gateway.on_gateway_status(GatewayConnectionStatus.ERROR,
                                           f"交易服务器登录失败: {error.get('ErrorMsg', '未知错误')}")
    ...
    def onRspQryInstrument(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        try:
            if error.get("ErrorID"):
                self.gateway.write_log(_("TtsTdApi：onRspQryInstrument 出错。最后：{}，错误 ID：{}").format(last, error.get('ErrorID', 'N/A')))
            # 1. 合约对象构建
            ...
            if last:
                ...
                self.gateway.write_log(_("TtsTdApi：onRspQryInstrument 已完成，合约信息查询成功"))
    ...
    def onRspQryInstrumentCommissionRate(self, data: dict, error: dict, reqid: int, last: bool):
        ...
        """
        if error.get("ErrorID"):
            self.gateway.write_error(_("TtsTdApi：OnRspQryInstrumentCommissionRate 查询失败。错误 ID：{}").format(error.get('ErrorID', 'N/A')),
                error)
            return
        ...
        if data is None or not data.get("InstrumentID"):
            ...
            if last:
                self.gateway.write_log(_("TtsTdApi：OnRspQryInstrumentCommissionRate 收到无效或空的合约手续费数据（最后一条）。ReqID: {}").format(
                        reqid))
            return
