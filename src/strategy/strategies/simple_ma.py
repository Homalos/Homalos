...
@Email      : donnymoving@gmail.com
@Software   : PyCharm
@Description: 双均线交叉策略 - 基于快慢均线交叉的简单趋势跟踪策略
"""
...
class MACrossStrategy(SpecificStrategyApi):
    ...
    def __init__(self):
        ...
        self.last_cross_signal: dict[str, str | None] = {ins: None for ins in self.instruments}  # "golden" or "death"
    ...
    def on_bar(self, bar: BarData) -> None:
        ...
        if len(self.bars[instrument_id]) < self.slow_period:
            self.logger.info(
                f"[{instrument_id}] K线数量不足 ({len(self.bars[instrument_id])}/{self.slow_period})，等待更多数据...")
            return
