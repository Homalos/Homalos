...
class DataCenterService:
    ...
    async def start(cls, user_id: int, db) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取start_data_center.py的绝对路径
            script_path = Path("start_data_center.py").absolute()
            ...
            logger.info(f"数据中心启动成功 PID={process.pid}")
    ...
    async def stop(cls, user_id: int, db, force: bool = False, timeout: int = 30) -> Dict[str, Any]:
        ...
        pid = cls.get_pid()
        if not pid:
            return {"success": False, "message": "数据中心未运行"}
        ...
        try:
            ...
            if force:
                ...
                process.kill()
                action = "强制停止"
                logger.info(f"强制停止数据中心 PID={pid}")
            else:
                # 优雅停止（发送SIGTERM）
                process.terminate()
                action = "优雅停止"
                logger.info(f"发送SIGTERM信号到数据中心 PID={pid}")
            ...
            logger.info(f"数据中心已{action} PID={pid}")
            return {"success": True, "message": f"数据中心已{action}"}
