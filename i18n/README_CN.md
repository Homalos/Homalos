...
### ✨ 最新更新 (v0.0.3.20251012)

- 🎨 **前端组件化重构**：将 `Home.vue`（945行）拆分为7个独立组件
  - 仪表盘、控制台、策略管理、任务调度器、通知中心、系统设置、关于页面
  - Home.vue 瘦身92%（945 → 280行）
  - 单一职责原则，可维护性大幅提升
- 🔧 **系统配置管理**：
  - 开发模式和交易时间检查设置
  - 与 `config/system.yaml` 双向同步
  - 每次修改自动备份
  - 审计日志记录所有配置变更
- 📄 **关于页面动态化**：从配置文件加载系统信息
  - 消除硬编码，便于更新
  - 公开API访问（无需登录认证）
- 🐛 **Bug修复**：修复配置保存问题、优化验证逻辑
- 📚 **文档完善**：新增5份详细指南和API自动化测试
