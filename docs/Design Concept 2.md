...
我将深入分析data_service.py模块的架构和功能，系统性地梳理其核心组件、数据流转机制和业务逻辑。
...
filePath: d:/Project/PycharmProjects/Homalos_v2/src/services/data_service.py
          
继续查看DatabaseManager的核心方法和BarGenerator、BarManager类的实现。
...
查看数据服务的剩余核心功能，包括数据查询、持久化和其他事件处理方法。
...
继续查看数据服务的剩余功能，包括数据查询、统计和其他核心方法。
...
## 数据服务模块（DataService）功能分析

### 🏗️ 核心架构组件

**1. DatabaseManager（数据库管理器）**
- 管理SQLite数据库（trading.db），支持WAL模式提高并发性能
- 创建和维护数据表：tick_data、bar_data、orders、trades、positions
- 实现批量写入缓存机制和后台异步写入线程
- 提供数据查询接口，支持按时间范围、合约等条件查询

**2. BarGenerator（K线合成器）**
- 单合约单周期K线实时合成
- 根据tick数据生成不同周期的K线数据
- 支持开盘价、最高价、最低价、收盘价、成交量等完整K线信息

**3. BarManager（K线管理器）**
- 多合约多周期K线统一管理
- 支持1分钟、5分钟、10分钟等多种周期
- 将tick数据智能分发给对应的BarGenerator
...
**事件分发**
- 为每个策略发布专用事件：`market.tick.{strategy_id}`、`market.bar.{strategy_id}`
- 同时保持通用事件发布供全局监听
- 智能事件路由，确保数据准确送达
...
**事件处理器**
- 订阅请求处理（DATA_SUBSCRIBE）
- 取消订阅处理（DATA_UNSUBSCRIBE）
- 网关订阅成功/失败事件处理
- 数据查询请求处理
- 数据持久化请求处理
...
1. **模块化设计**：数据库、K线合成、订阅管理等功能独立封装
2. **高性能**：异步处理、批量写入、内存缓存等优化措施
3. **可扩展性**：支持多策略、多合约、多周期的灵活配置
4. **可靠性**：完善的错误处理、重试机制和状态跟踪
5. **实时性**：事件驱动架构确保数据实时处理和分发
...
## 数据中心按交易日分表存储改进完成
我已成功改进了数据中心的数据库架构，实现了按交易日+合约分表存储tick和bar数据的功能。

### 主要改进内容 1. 数据库架构重构
- 分表存储结构 ：
  - Tick数据： `tick_db` /tick_YYYYMMDD.db
  - Bar数据： `bar_db` /bar_YYYYMMDD.db
- 按日期自动创建 ：每个交易日自动创建独立的数据库文件
- 保持表结构一致 ：每个日期数据库都包含完整的表结构和索引 2. 批量写入优化
- 按日期分组缓存 ：使用 defaultdict(list) 按日期分组管理批量写入缓存
- 事务批量提交 ：
  - Tick数据：每8,000条记录commit一次
  - Bar数据：每5,000条记录commit一次
- 线程安全 ：使用锁机制确保多线程环境下的数据一致性 3. 查询功能增强
- 跨日期查询 ： `query_tick_data` 和 `query_bar_data` 支持跨多个日期数据库文件查询
- 智能日期范围 ：根据查询条件自动确定需要查询的日期范围
- 结果合并排序 ：自动合并多个数据库的查询结果并按时间排序 4. 核心方法更新
- `_init_daily_database` ：按需初始化指定日期的数据库文件
- `_flush_daily_batch` ：刷新指定日期的批次数据到对应数据库
- `_add_to_daily_batch` ：添加数据到按日期分组的批量写入缓存
### 验证结果
通过实际运行测试，新的分表存储功能运行正常：

- ✅ 成功创建了 tick_20250715.db 和 bar_20250715.db 数据库文件
- ✅ Tick数据库已存储192条记录，Bar数据库存储2条记录
- ✅ 数据正在按批次正常写入，性能良好
- ✅ Parquet文件也按日期分别存储，保持双重备份
### 架构优势
1. 性能提升 ：按日期分表减少了单表数据量，提高查询和写入性能
2. 存储优化 ：每日独立文件便于数据管理和归档
3. 扩展性强 ：支持历史数据查询和未来数据的无缝添加
4. 维护便利 ：可以按日期进行数据备份、清理和维护操作
5. 容错能力 ：单日数据库问题不会影响其他日期的数据
数据中心现在具备了更强的数据处理能力和更好的可维护性，为高频交易数据的长期存储和查询提供了坚实的基础。

---

## Parquet存储方式改进完成
已成功改进Parquet存储方式，实现了按照您要求的新存储结构：

### 🎯 改进内容
新的存储结构：

- tick_parquet/ - tick数据主目录
  - 20250715/ - 按交易日建立的子目录
    - FG509.parquet - 按合约名称存放的数据文件
    - SA509.parquet - 按合约名称存放的数据文件
- bar_parquet/ - bar数据主目录
  - 20250715/ - 按交易日建立的子目录
    - FG509.parquet - 按合约名称存放的数据文件
    - SA509.parquet - 按合约名称存放的数据文件
### 🔧 技术实现
...
1. _init_parquet_storage() - 初始化tick_parquet和bar_parquet主目录
2. _flush_tick_parquet_buffer() - 按日期和合约分组写入tick数据
3. _flush_bar_parquet_buffer() - 按日期和合约分组写入bar数据
存储逻辑优化：
...
- ✅ tick数据正确存储到 tick_parquet/20250715/FG509.parquet 等文件
- ✅ bar数据正确存储到 bar_parquet/20250715/FG509.parquet 等文件
- ✅ 数据文件按合约分离，便于查询和管理
- ✅ 目录结构清晰，支持按日期和合约快速定位
### 🚀 优势
存储优势：

- 精确定位 - 可直接访问特定日期特定合约的数据
- 并行处理 - 不同合约数据独立，支持并行读写
- 存储效率 - 避免大文件，提高I/O性能
- 维护便利 - 可单独备份或清理特定合约数据
新的Parquet存储结构已完全按照您的要求实现，数据中心正常运行并持续写入数据到新的存储结构中。

---

## 按合约分表数据库架构重构完成
我已成功完成了数据中心数据库从按日期分表到按合约分表的架构重构，主要修改包括：

### 核心架构变更
1. 表结构策略调整
   
   - 从按日期分表改为按合约分表
   - 每个合约独立建表： tick_{symbol}_{exchange} 和 bar_{symbol}_{exchange}
   - 数据库文件按日期分库： tick_YYYYMMDD.db 和 bar_YYYYMMDD.db
2. 批量写入机制重构
   
   - 修改了 `save_tick_data` 和 `save_bar_data` 方法
   - 批量缓存从按日期分组改为按合约和日期分组
   - 使用 _add_to_contract_batch 替代原来的 _add_to_daily_batch
3. 查询方法优化
   
   - 更新了 `query_tick_data` 和 `query_bar_data` 方法
   - 根据合约和交易所动态获取表名
   - 移除SQL查询中的 symbol 和 exchange 条件
   - 添加表存在性检查
### 主要修改文件
- `data_center_database.py` ：核心数据库管理类
  
  - 添加了logger初始化
  - 重构了表创建、批量写入和查询逻辑
  - 修复了Parquet写入方法中的logger引用问题
  - 更新了状态查询方法以支持新架构
- `test_contract_tables.py` ：测试脚本
...
- 数据库状态查询正常
### 架构优势
1. 性能提升 ：每个合约独立表，避免了大表查询性能问题
2. 扩展性好 ：新合约自动创建表，支持动态扩展
3. 维护便利 ：可以针对单个合约进行数据维护
4. 查询优化 ：无需在SQL中过滤合约，查询效率更高
按合约分表的数据库架构现已完全就绪，可以支持高效的期货行情数据存储和查询。

---

## 合约表命名优化完成
我已成功优化了 `data_center_database.py` 中的合约表命名架构，实现了以下关键改进：

### 主要优化内容
1. 表名简化

- 原来： tick_ag2412_SHFE 、 bar_ag2412_SHFE
- 现在：直接使用合约名 ag2412
- 通过修改 `_get_table_name` 方法实现
2. 统一表结构

- 每个合约只创建一张表，同时存储tick和bar数据
- 通过 data_type 字段区分数据类型（'tick' 或 'bar'）
- 优化了主键设计： (data_type, datetime, interval)
3. 批量写入优化

- 统一了 `_flush_contract_batch` 方法的逻辑
- 在SQL插入语句中添加了 data_type 字段
- 保持了高效的批量写入性能
4. 查询方法更新

- 修正了 `query_tick_data` 和 `query_bar_data` 方法
- 在查询条件中添加了 data_type 过滤
- 确保查询结果的准确性
### 测试验证
创建并运行了 `test_optimized_contract_tables.py` 测试脚本，验证了：

- ✅ 数据保存功能正常
- ✅ 表名已简化为合约名（ag2412、a2509、rb2501）
- ✅ 统一表结构包含所需字段
- ✅ 数据类型区分机制工作正常
### 架构优势
1. 简洁性 ：表名更加直观，便于理解和维护
2. 统一性 ：每个合约只有一张表，减少了表的数量
3. 扩展性 ：通过 data_type 字段可以轻松支持更多数据类型
4. 性能 ：保持了原有的高效批量写入和查询性能
5. 维护性 ：简化了数据库结构，降低了维护复杂度
优化后的架构更加清晰高效，为后续的功能扩展和维护提供了良好的基础。

---

## 数据中心 'no such table' 错误修复完成
### 问题诊断
通过分析错误日志，我发现问题的根本原因是 表缓存机制存在缺陷 。在 `data_center_database.py` 中， _create_contract_table 方法使用 (date_str, table_name) 作为表缓存的键，但没有区分数据类型（tick 或 bar）。

这导致了一个严重的竞态条件：

- 当系统在 tick 数据库中创建了某个合约的表后，缓存会记录该表已创建
- 但当需要在 bar 数据库中创建同名表时，系统误认为表已存在，跳过了创建步骤
- 结果是 bar 数据写入时找不到对应的表，产生 "no such table" 错误
### 修复方案
我对表缓存机制进行了以下关键修改：

1. 更新缓存键格式 ：将表缓存键从 (date_str, table_name) 改为 (data_type, date_str, table_name) ，确保 tick 和 bar 数据库的表创建状态独立跟踪
2. 修改缓存检查逻辑 ：在 _create_contract_table 方法中，表缓存检查现在包含数据类型，避免了不同数据库间的缓存冲突
3. 更新注释文档 ：同步更新了代码注释，反映新的缓存键格式
### 验证结果
修复后重新启动数据中心，测试结果显示：

- ✅ 成功订阅 935 个合约
- ✅ 正常处理 tick 数据（已处理 8000+ 个 tick）
- ✅ 完全消除了 "no such table" 错误
- ✅ tick 和 bar 数据都能正常写入各自的数据库
### 技术改进
这次修复不仅解决了当前问题，还提升了系统的整体稳定性：

- 消除竞态条件 ：不同数据类型的表创建现在完全独立
- 提高并发安全性 ：避免了多线程环境下的缓存冲突
- 增强可维护性 ：更清晰的缓存键结构便于调试和扩展
数据中心现在可以稳定运行，所有合约的 tick 和 bar 数据都能正确保存到对应的数据库表中。